...
- The questions and options must mention recognized workflow evidence in the user's language, for example "角色锚点 + 道具锚点 + 分镜草图 + 逐镜视频", rather than generic "当前案例". Only mention visual style examples such as "皮克斯 3D 卡通广告片" or "柔和棚拍光影" when those are actually supported by canvas prompt/media evidence.
...
- Every Skill must include allowed_recipe_ids containing exactly the reusable Recipe ids available to that Skill. Each id must refer to a top-level Recipe draft in the same draft session or an intentionally reused saved Recipe.
