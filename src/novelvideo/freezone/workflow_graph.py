...
from novelvideo.freezone.workflows import registered_workflows, workflow_aliases, workflow_by_type
...
LINK_TYPE_RULES = {
    ...
    "visual_reference_for": ({"ImageNode", "VideoNode"}, {"ImageNode", "VideoNode"}),
    "source_media_for": (
...
def _infer_link_type(
    ...
    if source_object in {"ImageNode", "VideoNode", "AudioNode"} and target_object in {
        ...
        return "source_media_for"
    if source_object in {"ImageNode", "VideoNode"} and target_object in {"ImageNode", "VideoNode"}:
        return "visual_reference_for"
    return None
