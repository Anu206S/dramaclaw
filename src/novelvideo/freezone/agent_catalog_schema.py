...
class AgentCatalogSkillConfig(_CatalogBaseModel):
    ...
    @model_validator(mode="after")
    def validate_dynamic_or_template_contract(self) -> "AgentCatalogSkillConfig":
        if not self.allowed_recipe_ids and not self.workflow_templates:
            raise ValueError("skill must include allowed_recipe_ids or workflow_templates")
        return self
