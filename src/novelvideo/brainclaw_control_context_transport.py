...
* the *grouping* key mints the opaque episode/project IDs and must outlive
  signing-key rotation, because rotating it splits one episode into two
...
class ControlContextScope:
    ...
    ``episode_id`` and ``project_id`` are raw internal identifiers. They are
...
class ControlContextRuntime:
        ...
        *,
        ...
        if grouping_key_path is not None:
            ...
            if self.grouping_key == self.signing_key:
                # Sharing one secret would tie the episode identity's lifetime to
    ...
    def next_ordinal(self, episode_group_id: str) -> int:
        # Per-episode and monotonic *as issued*. Concurrency and retries mean
        ...
        with self._ordinal_lock:
            ordinal = self._ordinals.get(episode_group_id, 0)
            self._ordinals[episode_group_id] = ordinal + 1
            return ordinal
    ...
    def header_for(
        ...
        if isinstance(scope, OpaqueControlContextScope):
            episode = scope.episode_group_id
            ...
            episode = group_id(self.grouping_key, "episode", scope.episode_id)
        ...
        context = ControlContext(
            ...
            checkpoint_ordinal=self.next_ordinal(episode),
...
async def sign_brainclaw_control_context(request) -> None:
    ...
    try:
        ...
        # degrade this request to diagnostic evidence, not fail the episode.
        return
