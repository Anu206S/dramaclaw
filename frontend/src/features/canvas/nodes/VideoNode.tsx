...
function isVideoModeSupportedByModel(
  mode: VideoGenMode,
  model: { id?: string; apiModel?: string; supportedModes?: string[] } | null | undefined,
): boolean {
  const configured = model?.supportedModes ?? [];
  if (configured.length > 0) {
    ...
    return configured.includes(modeKey[mode]);
  }
...
export const VideoNode = memo(
  ({ id, data, selected, width, height }: VideoNodeProps) => {
    ...
    const selectedModelReferenceError = videoModelReferenceDisabledReason(
      selectedVideoModel,
      upstreamCounts,
      genMode,
    );
                ...
                <div className="flex min-w-0 items-center gap-2">
                  <ProviderModelPicker
                    ...
                    getOptionDisabledReason={(model) =>
                      videoModelReferenceDisabledReason(model, upstreamCounts, genMode)
                    }
                  />
                  <VideoConfigChip
