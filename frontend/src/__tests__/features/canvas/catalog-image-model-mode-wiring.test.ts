...
function sharedSource(name: string): string {
  return readFileSync(
    resolve(process.cwd(), `src/features/canvas/nodes/shared/${name}.ts`),
    "utf8",
  );
}
...
describe("图片节点递给模型参数控件的模式", () => {
  ...
  it("ImageGenNode 按有无参考图推导模式", () => {
    const source = sharedSource("useImageGenerationForm");
    expect(source).toContain("const generationMode =");
    expect(source).toContain(
      "orderedReferenceUrls.length > 0 ? 'image_to_image' : 'text_to_image'",
    );
    expect(source).toContain("modelParamsMode: generationMode");
    expect(source).toContain("genMode: generationMode");
  });
  ...
  it("没有任何一个图片节点还把裸 data.generationMode 递给控件", () => {
    expect(chipBlock(nodeSource("ImageEditNode"))).not.toContain("data.generationMode");
    expect(sharedSource("useImageGenerationForm")).not.toContain(
      "modelParamsMode: data.generationMode",
    );
    expect(chipBlock(nodeSource("StoryboardGenNode"))).not.toContain("data.generationMode");
  });
});
