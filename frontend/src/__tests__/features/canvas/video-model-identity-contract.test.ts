...
/**
 * 视频生成表单里有两个模型标识：`modelId`(= selectedVideoModel.id，EE 下是 ULID，只用于
 ...
 */
describe("useVideoGenerationForm feeds capability helpers the api model, not the catalog id", () => {
  it("isSeedance20Model 走 selectedVideoModelId", () => {
    expect(generationFormSource).toContain(
    ...
    expect(generationFormSource).not.toContain("isSeedance2VideoModel(modelId)");
  });
  ...
  it("modelId 只作为 catalogId 的兜底出现在提交参数里", () => {
    expect(generationFormSource).toContain("selectedVideoModel?.catalogId ?? modelId");
  });
});
