...
def test_cognee_gateway_structured_output_disables_reasoning(monkeypatch):
    ...
    async def fake_graph_read(*args, **kwargs):
        return [SimpleNamespace(search_result="人物图谱上下文")]

    monkeypatch.setattr(LLMGateway, "acreate_structured_output", fake_structured_output)
    monkeypatch.setattr(pipeline, "_run_graph_read", fake_graph_read)

    assert asyncio.run(pipeline.extract_characters_from_graph()) == []
    assert calls == [
        {
