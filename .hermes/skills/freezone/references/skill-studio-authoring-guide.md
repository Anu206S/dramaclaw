...
- 首轮问题可以问“保留具体案例还是抽象成模板”“哪些硬约束要保留”“复用方式/工作流细致程度如何”。
...
- **复用方式**：选择“细粒度复用（推荐）”还是“简化复用”。细粒度复用会把角色、道具、分镜、视频片段、音频等关键步骤分别沉淀，之后更容易单独复用和调整；简化复用会合并成较少步骤，配置更轻，但后续单独调整某一步的灵活性较弱。不要在问题文案里说 Recipe，也不要把最终合成作为用户要选择的粒度项。
